function correct_rate = SVM_classifier_fusion(train1,train2,train_labels,test1,test2,test_labels,blocksize,hopsize,alpha,normal)
%
if nargin < 10
    normal = 1;
end
...
for i = 1: length(train2)
    ...
    for j = 1:hopsize:len
        if j + blocksize <= len
            m = mean(train2{i}(j:j+blocksize,:));
            var = std(train2{i}(j:j+blocksize,:));
            dif = diff(train2{i}(j:j+blocksize,:));
            dif2 = diff(dif);
            m_diff = mean(dif);
            m_var = std(dif);
            m_dif2 = mean(dif2);
...
SVMstruct1 = svmtrain(train_y,train_HPSS,['-b 1']);
%}
...
for i = 1:length(test1)
    ...
    for j = 1:hopsize:len
        if j + blocksize <= len
            m = mean(test2{i}(j:j+blocksize,:));
            var = std(test2{i}(j:j+blocksize,:));
            dif = diff(test2{i}(j:j+blocksize,:));
            dif2 = diff(dif);
    ...
    %}
    prob = alpha *mean(prob1) +  mean(prob2) + (alpha-1) *mean(prob3) ;
    [m,predict] = max(prob');
