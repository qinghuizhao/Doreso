function [cf,correct_rate] = SVM_fusion_stack2(train1,train_labels,validation1,validation_labels,test1,test_labels,blocksize,hopsize,normal)
%
cf = zeros(10,10);
...
for i = 1: length(train1)
    ...
    for j = 1:hopsize:len
        if j + blocksize <= len
            temp = [train1{i}(j:j+blocksize,:)];
            m = mean(temp);
            var = std(temp);
            dif = diff(temp);
            m_diff = mean(dif);
            m_var = std(dif);
            com = [m,var,m_var,m_diff];
...
SVMstruct = svmtrain(train_y,train_x,'-b 0');
...
for i = 1:length(validation1)
    ...
    for j = 1:hopsize:len
        if j + blocksize <= len
            temp = [validation1{i}(j:j+blocksize,:)];
            m = mean(temp);
            var = std(temp);
            dif = diff(temp);
            m_diff = mean(dif);
    ...
    [predict,acc,prob] = svmpredict(val_label_temp,val_input,SVMstruct,'-b 0');
    
%     for k = 1:length(predict)
%         prob(k,:) = prob(k,:) .* exp(validation_data(i,:));
%     end
%     n_dim = 10;
%     nn_dim = 20;
...
for i = 1:length(test1)
    ...
    [predict,acc,prob] = svmpredict(test_label,test_input,SVMstruct,'-b 0');
