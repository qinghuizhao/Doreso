function [res,correct_rate] = SVM_classifier_fusion_new(train1,train2,train_labels,test1,test2,test_labels,blocksize,hopsize,alpha,normal)
%
if nargin < 10
    normal = 1;
end
...
for i = 1:length(test1)
    ...
    %}
    prob = 65 *mean(prob1) + 53 *  mean(prob2) +  59 *mean(prob3) ;
    [m,predict] = max(prob');
