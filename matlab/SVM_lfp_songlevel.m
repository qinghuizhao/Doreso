function [cf,correct_rate] = SVM_lfp_songlevel(input,blocksize,hopsize,normal,thres)
%
if nargin < 4
    normal = 1;
    thres = 0.5 ; 
...
for i = 1: length(train_mfccP_x)
    ...
    lfp_train = train_sample{i};
    lfp = median(lfp_train(:,1:n_dim)) * 0.6;
    temp = [];
    for j = 1:hopsize:len
        if j + blocksize <= len
...
save ./data/bestPara bestc bestg;
%}
%% testphase
...
for i = 1:length(test_mfccP_x)
    test_input = [];
    test_label = [];
    lfp_test = test_sample{i};
    lfp = median(lfp_test(:,1:n_dim)); 
